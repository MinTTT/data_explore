'''
usage:
od table have column names: Number,Time,OD600,Sample,Dilution_Factor
flow cytometry column names: 'Tube Name:' has same values correspond to 'Number' in od table
'''
...
# %% set data dir
data_save_path = r'./20200929_medium_shift'
data_raw_path = r'./20200929_medium_shift/20200723_down_shift_glycerol.csv'
...
data_raw = pd.read_csv(data_raw_path, comment='#')
...
for index, ax in enumerate(fig1.axes):
    ...
    ax.scatter(data_select['Time_h'][diluation_mask], [1e-1] * len(data_select['Time_h'][diluation_mask]),
               marker='^', facecolors='k', edgecolors='k')
    # ax2[y_index, x_index].legend()
    ax.grid(False)
    ax.set_xlim(0, data_summary['Time_h'].max() * 1.1)
