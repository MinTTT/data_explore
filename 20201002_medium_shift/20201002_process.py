...
#%% set data dir
data_raw_path = r'./20201002_medium_shift/20201002_medium_shift.csv'
fcs_dir = r'E:\Fu_lab_Data\cytometer data\20201002_down_shift_trimed'
flowcyto_stat = pd.read_csv(r'./20201002_medium_shift/20201002_medium_shift_flow_data.csv', skiprows=2, na_values='####')
#%%
...
for index, samp in enumerate(sample_list):
    ...
    gr_index = gr >= 0.001
    diluation_mask = [True if ~np.isnan(factor) else False for factor in data_select['Dilution_Factor']]
    ax3[x_index].scatter(data_select['Time_h'], data_select['P1 AND NOT(H2-LL) Mean Red-H'], marker='D',
                                  facecolors='None', edgecolors='r')
    ax3[x_index].scatter(data_select['Time_h'][diluation_mask], [0]*len(data_select['Time_h'][diluation_mask]),
                                  marker='^', facecolors='k', edgecolors='k')
    ax3_y = ax3[x_index].twinx()
    ax3_y.scatter(data_select['Time_h'], data_select['P1 AND NOT(H2-LL) Mean Green-H'], marker='D',
                  facecolors='None', edgecolors='g')

    ax3[x_index].grid(False)
    ax3[x_index].set_ylabel('Red Channel (a. u.)', color='red')
    ax3_y.set_ylabel('Green Channel (a. u.)', color='green')
    ax3[x_index].set_xlim(0.001, 35)
    ax3[x_index].set_ylim(-25, 450)
    ax3[x_index].set_xlabel('Time (h)')
    ax3[x_index].set_title(samp)
...
for samp in sample_list:
    ...
    ani.save(f'./20201002_medium_shift/animation_{samp}.avi', writer='ffmpeg', fps=4)
# fig4.show()
